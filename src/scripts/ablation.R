...
epsilon <- 0.1         
M <- train_data$M # Number of conditions.
...
mu0_base[is_u]     <- (df$unif_a[is_u] + df$unif_b[is_u]) / 2  # Get the Uniform mean.
sigma20_base[is_u] <- (df$unif_b[is_u] - df$unif_a[is_u])^2 / 12 # Get the Uniform variance.

mix_mean <- df$mix_w1*df$mix_mu1 + (1 - df$mix_w1)*df$mix_mu2 # Collapse the mixture mean.
mix_var  <- df$mix_w1*(df$mix_sd1^2 + (df$mix_mu1 - mix_mean)^2) + # Collapse the mixture variance.
            (1 - df$mix_w1)*(df$mix_sd2^2 + (df$mix_mu2 - mix_mean)^2)
mu0_base[is_m]     <- mix_mean[is_m] # Store the mixture mean.
sigma20_base[is_m] <- mix_var[is_m] # Store the mixture variance.

make_length_bins <- function(x) {
  # Generate tertiles for the prior mean of delays.
...
make_tight_bins <- function(x) {
  # Generate tertiles for the prior variance of delays.
  q <- quantile(x, probs = c(0, .33, .66, 1), na.rm = TRUE)
  if (anyDuplicated(q)) {
    r <- range(x, na.rm = TRUE)
    if (!is.finite(r[1]) || r[1] == r[2]) {
...
delay_family <- df$delay_dist # Keep the delay family per condition.
delay_group  <- if (!is.null(df$delay_group)) as.character(df$delay_group) else as.character(df$delay_dist)  
length_bins  <- make_length_bins(mu0_base) # Bin conditions by expected delay length.
tight_bins   <- make_tight_bins(sigma20_base) # Bin conditions by prior tightness.
...
make_view <- function(d_row, t_row, rho_i, tau_i, M) {
  # Partition observed states with respect to the observation window.
...
make_view_subset <- function(d_row, t_row, rho_i, tau_i, M, cols) {
  # Create a view then mask the indices to a chosen subset.
...
hungarian_align <- function(pred, truth, K) {
  # Align predicted cluster labels to true labels via the Hungarian algorithm.
...
run_one_setting <- function(var_scale) {
  # Run training, prediction, shrinkage and sensitivity for one prior variance scale.
  ...
  mu0     <- mu0_base                                      
  sigma20 <- pmax(sigma20_base * var_scale, 1e-8) # Scale the prior variances.
  ...
  if (file.exists(cache_train)) {
    ...
    if (!is.null(pp_chk$gap_sigma2_star)) need_train <- FALSE  # Skip training if shrinkage fields exist.
  }
  if (need_train) {
    ...
    )  # Fit the delay aware VB model at this prior strength.
    saveRDS(posterior_train, cache_train)  # Save the trained posterior for reuse.
  }

  # EXTRACT PAARMETERS FOR PREDICTION
  ...
  denom_w <- pmax(colSums(ed_tr, na.rm = TRUE), 1e-8) # Build weights for presence weighting.

  # Unweighted.
  SR   <- colMeans(gap_var_tr, na.rm = TRUE) / sigma20 # Compute shrinkage ratio by condition.
  PP   <- (colMeans(gap_mu_tr,  na.rm = TRUE) - mu0) / sqrt(sigma20) # Compute prior pull by condition.

  # Presence-weighted.
  SR_w <- (colSums(gap_var_tr * ed_tr, na.rm = TRUE) / denom_w) / sigma20  # Compute presence weighted shrinkage.
  PP_w <- ((colSums(gap_mu_tr  * ed_tr, na.rm = TRUE) / denom_w) - mu0) / sqrt(sigma20) # Compute presence weighted pull.
  ...
  N_test <- test_data$N                                       
  phi_mat <- matrix(NA_real_, N_test, K) # Allocate cluster posteriors.
  for (i in 1:N_test) {
    # Build a view for the full observation window and compute predictive phi.
    ...
    phi_mat[i, ] <- pred$phi  # Record the cluster weights.
  }
  ...
  ari <- mclust::adjustedRandIndex(raw_pred, truth_clusters)  # Unpermuted ARI.
  nmi <- aricode::NMI(raw_pred, truth_clusters)               # Unpermuted NMI.
  ...
  phi_pre_mat <- matrix(NA_real_, N_test, K) # Allocate pre cut phi.
  pred_list_pre <- vector("list", N_test)                     
  for (i in 1:N_test) {
    # Build a view up to the cut and compute predictive quantities.
    cut_i <- cut_ages[i]
    ...
    phi_pre_mat[i, ] <- pred$phi # Save pre cut cluster weights.
    pred_list_pre[[i]] <- pred # Save predictive parameters for mixing.
  }

  P_after <- matrix(0, N_test, M)           
  E_t_after <- matrix(NA_real_, N_test, M)  
  for (i in 1:N_test) {
    # Mix after cut presence and expected times to the end of observation.
    cut_i <- cut_ages[i]
    T_i   <- test_data$tau[i]
  ...
  y_pred <- as.integer(y_prob >= 0.5) # Threshold for accuracy.

  acc_after   <- mean(y_pred == y_true) # Compute after cut accuracy.
  auroc_after <- safe_auc(y_true, y_prob) # Compute after cut AUROC.

  mae_mask <- is_pos & !is.na(E_t_after) # Restrict MAE to observed after cut events.
  mae_vals <- abs(E_t_after[mae_mask] - t_true[mae_mask]) # Compute absolute errors for diagnosis time.
  mae_after <- if (length(mae_vals) > 0) mean(mae_vals) else NA_real_  # Average the errors.

  # DROP-SET SENSITIVITY: STRONG vs WEAK PRIORS ONLY
  # Re-run predictive using only strong or only weak prior conditions as evidence.
  pred_list_pre_strong <- vector("list", N_test)  
  pred_list_pre_weak   <- vector("list", N_test)  
  for (i in 1:N_test) {
    # Compute predictive quantities using only strong evidence.
    cut_i <- cut_ages[i]
    ...
    # Compute predictive quantities using only weak evidence.
  ...
  for (i in 1:N_test) {
    # Mix to compute strong and weak only predictions to the end of the window.
    cut_i <- cut_ages[i]
    T_i   <- test_data$tau[i]
  ...
  # Evaluate strong-only predictions on strong columns.
  ...
  # Evaluate weak-only predictions on weak columns.
  ...
  fam_levels <- unique(delay_family)  # Get unique delay families.
  len_levels <- c("short","medium","long") # Use three bins for length.
  strat_rows <- list()                             

  # By family
  for (fam in fam_levels) {
    # Evaluate metrics restricted to a given delay family.
  ...
  for (lev in len_levels) {
    # Evaluate metrics restricted to a length bin of the delay prior mean.
...
# RUN GRID & SAVE
all_results <- lapply(var_scales, run_one_setting)  # Run the ablation across prior strengths.
